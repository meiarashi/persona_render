...
function randomizeDetailSettingsFields() {
    ...
    // filterRandomValuesForAge関数のラッパー（モジュールを使用）
    function filterRandomValuesForAgeWrapper(list, ageInYears, categoryType) {
        // モジュールが利用可能な場合はそれを使用
        if (window.PersonaAgeFilters && window.PersonaAgeFilters.filterRandomValuesForAge) {
    ...
    // 最近の悩み/関心 (Concerns)
    let filteredConcerns = filterRandomValuesForAgeWrapper(personaRandomValues.concerns, ageInYears, 'concerns');
    ...
    const filteredFavoritePerson = filterRandomValuesForAgeWrapper(personaRandomValues.favorite_person, ageInYears, 'favorite_person');
    setRandomValueIfEmpty("favorite_person", getRandomItem(filteredFavoritePerson));

    // よく見るメディア/SNS (Media/SNS)
    const filteredMediaSNS = filterRandomValuesForAgeWrapper(personaRandomValues.media_sns, ageInYears, 'media_sns');
    ...
    const filteredHealthActions = filterRandomValuesForAgeWrapper(personaRandomValues.health_actions, ageInYears, 'health_actions');
    setRandomValueIfEmpty("health_actions", getRandomItem(filteredHealthActions));

    // 休日の過ごし方 (Holiday Activities)
    let filteredHolidayActivities = filterRandomValuesForAgeWrapper(personaRandomValues.holiday_activities, ageInYears, 'holiday_activities');
    ...
    // キャッチコピー (Catchphrase)
    const filteredCatchphrase = filterRandomValuesForAgeWrapper(personaRandomValues.catchphrase, ageInYears, 'catchphrase');
    setRandomValueIfEmpty("catchphrase", getRandomItem(filteredCatchphrase));

    // 最終的な論理チェック（モジュールを使用）
    if (window.PersonaValidation && window.PersonaValidation.validatePersonaConsistency) {
        window.PersonaValidation.validatePersonaConsistency();
    }
    
    console.log("[DEBUG] randomizeDetailSettingsFields: Completed.");
}

// validatePersonaConsistency 関数は persona-validation.js モジュールに移動しました

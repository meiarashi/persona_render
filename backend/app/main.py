...
def build_prompt(data, limit_personality="100", limit_reason="100", limit_behavior="100", 
    ...
    prompt_parts.append("\n## 重要：文字数制限について")
    prompt_parts.append("**各項目は指定された文字数を必ず守ってください。文字数は厳守です。**")
    prompt_parts.append("- 指定文字数の±10%以内に収めること")
    ...
    prompt_parts.append(f"\n1. **性格（価値観・人生観）**: 【厳守：{limit_personality}文字】")
    prompt_parts.append("   - その人の基本的な性格、物事への考え方、人生で大切にしている価値観を記述")
    prompt_parts.append(f"2. **通院理由**: 【厳守：{limit_reason}文字】")
    prompt_parts.append("   - 現在抱えている症状や健康上の問題、なぜ医療機関を訪れるのかを具体的に記述")
    prompt_parts.append(f"3. **症状通院頻度・行動パターン**: 【厳守：{limit_behavior}文字】")
    prompt_parts.append("   - どの程度の頻度で通院するか、症状が出た時の行動パターン、医療機関の利用傾向を記述")
    prompt_parts.append(f"4. **口コミの重視ポイント**: 【厳守：{limit_reviews}文字】")
    prompt_parts.append("   - 医療機関を選ぶ際に口コミのどの部分を重視するか、どんな情報を探すかを記述")
    prompt_parts.append(f"5. **医療機関への価値観・行動傾向**: 【厳守：{limit_values}文字】")
    prompt_parts.append("   - 医療機関や医師に対する基本的な姿勢、診療時の行動傾向、医療への期待を記述")
    prompt_parts.append(f"6. **医療機関に求めるもの**: 【厳守：{limit_demands}文字】")

...
class EStatIntegratedServiceV2:
    ...
    async def _get_medical_facilities_data(self, area_code: str) -> Dict:
        """医療施設データ取得"""
        cache_key = f"medical_{area_code}"
        ...
        try:
            async with aiohttp.ClientSession(timeout=self.timeout) as session:
                ...
                stats_code = self.master_data.get("api_config", {}).get("stats_tables", {}).get("medical_facilities", {}).get("statsCode", "00450021")
                ...
                async with session.get(search_url, params=search_params) as response:
                    if response.status == 200:
                        ...
                        # データ処理ロジック（後で実装）
                        result = {"facilities_count": 50, "from_api": True}
    ...
    async def get_regional_data(self, address: str) -> Dict[str, Any]:
        ...
        # 人口データを取得（実際のAPIを使用、住所も渡す）
        population_data = await self._get_simple_population_data(area_code, address)
    ...
    async def _get_simple_population_data(self, area_code: str, address: str = "") -> Dict:
        ...
        try:
            async with aiohttp.ClientSession(timeout=self.timeout) as session:
                # 統計コードを使用して国勢調査データを検索
                search_url = f"{self.base_url}/getStatsList"
                
                # マスターデータから統計コードを取得
                stats_code = self.master_data.get("api_config", {}).get("stats_tables", {}).get("population", {}).get("statsCode", "00200521")
                
                # 地域名を検索ワードに使用（地域コードより地域名の方が確実にヒットする）
                search_word = address if address else "東京都"
                
                search_params = {
                    "appId": self.api_key,
                    "statsCode": stats_code,  # 統計コードで検索
                    "searchWord": search_word,  # 地域名で検索
                    "limit": 10
                }
                
                logger.info(f"人口データ検索: statsCode={stats_code}, area={search_word}")
